...
void Match::join(uint32_t soldier_id, uint8_t soldier_type) {
    ...
    soldier->setRandomPosition(std::ref(soldiers), std::ref(zombies), calculate_mass_center(), y_dim);
...
void Match::setZombie(uint32_t zombie_id, uint8_t zombie_type) {
    ...
    zombie->setRandomPosition(std::ref(soldiers), std::ref(zombies), calculate_mass_center(), y_dim);
...
void Match::kill_soldier(uint32_t soldier_id) {
        if (soldiers.count(soldier_id)>0) {
        std::shared_ptr<Soldier> &soldier = soldiers.at(soldier_id);
        // mal acceder al atributo directamente 
        soldier->alive = false;
    }
}
...
double Match::calculate_mass_center(void) {
    if (soldiers.size() == 0) return x_dim * 0.5;
    double sum = 0;
    double count = 0;
    for (const auto & soldier : soldiers) {
